...
gap2=10
gap3=10
gap4=20 # 평균값의 차이가 적절하게 나는지 확인하기 위해 필요한 변수들
myTeam=0 # 우리팀 스코어 저장할 변수 선언 및 초기화
redTeam=0 # 상대팀 스코어 저장할 변수 선언 및 초기화
...
while video.isOpened(): # 비디오가 열려있을 경우 계속 반복!(비디오가 열리기만 하면 무한 반복이다.)
    ...
    if(abs(np.average(avg_color_per_row, axis=1)[2]-avg_color1)>gap1 or abs(np.average(avg_color_per_row, axis=1)[16]-avg_color2)>gap2): # 전의 평균값과 차이가 많이 난다면(숫자가 바뀌었다면)
        myTeam+=1
    ...
    if(abs(np.average(avg_color_per_row2, axis=1)[2]-avg_color4)>gap4 or abs(np.average(avg_color_per_row2, axis=1)[16]-avg_color3)>gap3):
        ...
        tts = gTTS(text=text, lang='ko')
        print("red", redTeam, avg_color3) # 현재 수치 확인할 수 있도록 출력하기
        ...
        if(redTeam==2):
            ...
            avg_color4=24
            count+=1
            print("red 숫자 조정") # 현재 수치 확인할 수 있도록 출력하기
